...
- Users can drag-and-drop artifacts (stages, booths, facilities) onto the grid planner (free form; no snapping).
...
Note on visibility (v1 rule):
- If Step 2 location is set to "egen adresse" (custom), hide the planner entirely and require a single plan PDF upload instead. The planner is only available for location presets.
